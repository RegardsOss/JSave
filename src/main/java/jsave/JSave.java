...
public class JSave {
    ...
    private HashMap<String, Object> readRecord() throws Exception {
        ...
        long nextrec = Utils.read_uint32(this.raf);
...
//        System.out.println(nextrec);
    ...
    public void displayAvailableVariables() {
        ...
        for (Map.Entry<String, Object> entry : this.getVariables().entrySet()) {
            ...
            System.out.print("- " + entry.getKey());
            System.out.print("[<type '" + entry.getValue().getClass() + ">]");
    ...
    public static void main(String[] args) throws FileNotFoundException, IOException, Exception {
        ...
        String savFilePath = new File(".").getCanonicalPath() + File.separator+"data"+File.separator;
        JSave sav = new JSave(new File(savFilePath + "cube2982_4.sav"));
        sav.read();

        sav.displayFileMetadata();
        sav.displayAvailableVariables();
        
        Matrix carte = (Matrix) sav.getVariables().get("carte");
        Matrix longitudes = (Matrix) sav.getVariables().get("longi");
        Matrix latitudes = (Matrix) sav.getVariables().get("lati");
        Matrix wavelengths = (Matrix) sav.getVariables().get("wave");  
